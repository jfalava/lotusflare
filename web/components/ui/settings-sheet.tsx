...
const SettingItem: React.FC<SettingItemProps> = ({
  ...
  >
    <p className="text-sm font-medium text-foreground pr-4 flex-1 min-w-0">{label}</p>
...
interface SettingsSectionProps {
  title: string;
  children: React.ReactElement<SettingItemProps> | React.ReactElement<SettingItemProps>[];
  className?: string;
}
...
const InventoryScrollSettingToggle: React.FC<InventoryScrollSettingToggleProps> = ({
  currentPreference,
  onPreferenceChange,
}) => {
...
const KeybindsSettingToggle: React.FC<KeybindsSettingToggleProps> = ({ isShowing, onToggle }) => {
...
export function SettingsSheet() {
  const { infiniteScroll, setInfiniteScroll, hideKeybinds, setHideKeybinds } = useSettings();
  ...
  return (
    <Sheet>
      ...
      <SheetContent className={clsx("flex flex-col p-0", "w-full max-w-xs sm:max-w-sm")}>
